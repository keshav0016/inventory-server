...
function itemStatusReportEmail(){
    models.ticket.findAll({
        ...
        where: {item_type : 'consumables', createdAt : {lte: limitDate}}
    })
    .then(totalConsumable => {
        consumableTickets.push(...totalConsumable);
        return models.ticket.findAll({
            include : [{ model : models.users, attributes : ['first_name','last_name']}], 
            where: {item_type : 'assets', createdAt : {lte: limitDate}}
        })
    })
    .then(assets => {
    ...
    .then(consumablesassigned => {
        ...
        return models.consumables_purchased.findAll({
            include:[{model:models.consumables}], 
            where: {createdAt : {lte: limitDate}}
        })
    })
    ...
    .then(admins => {
       ...
       if(consumablesAssignedDetails.length !== 0) {
        ...
        var buffer = xlsx.build([
            {name:'Asset Requests',data:AssetRequests},
            {name:'Consumable Requests',data:ConsumableRequests},
            {name:'Asset Details',data:AssetPurchaseDetails},
            {name:'Asset Assigned Details',data:AssetAssignedDetails},
            {name: 'Asset Repair Details',data:AssetRepairDetails},            
            {name:'Consumables Details',data:ConsumableDetails},
            {name:'Consumables Assigned details',data:ConsumableAssignedDetails},
            {name:'Vendors',data:VendorsList},
            {name:"Employees",data:EmployeeList},
        ]);
